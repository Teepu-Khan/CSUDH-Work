...
public class Pages 
{
    ...
    {
        this.pageRef = a; // Number of page references input by user.
        this.range = b; // The range from 0 - b input by user.
        this.phyFrames = c; // The number of page frames input by user.
    ...
    public void Fifo() 
    {
        ...
        for(int i = 0; i < pageRef; i++) 
        {
            ...
            if(!frames.contains(refString[i]))
            {
                ...
                if(frames.size() == phyFrames){ // Will limit queue to the number of frames entered by user.
                    frames.poll();
                }
